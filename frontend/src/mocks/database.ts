  ...
  for (const asset of targetAssets) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      out.push({
        ...
        status: pickWeighted([["confirmed", 70], ["added", 15], ["changed", 10], ["removed", 5]]),
...
export const riskScores: RiskScore[] = [
  ...assets.filter(() => chance(0.5)).map((a, i) => ({
    ...
    entityId: a.id,
    score: randInt(5, 95),
    criticality: pickWeighted([["low", 30], ["normal", 40], ["high", 20], ["critical", 10]] as [RiskScore["criticality"], number][]),
    factors: [
      { name: "Finding severity", contribution: randInt(10, 40), explanation: "Open critical/high findings on this asset." },
      { name: "Exposure", contribution: randInt(5, 25), explanation: "Internet-facing with an open administrative surface." },
    ],
  ...
  ...investigations.filter(() => chance(0.4)).map((inv, i) => ({
    ...
    entityId: inv.id,
    score: randInt(10, 90),
    criticality: pickWeighted([["low", 20], ["normal", 30], ["high", 30], ["critical", 20]] as [RiskScore["criticality"], number][]),
    factors: [{ name: "Severity", contribution: randInt(20, 50), explanation: "Investigation severity and confidence." }],
    calculatedAt: hoursAgo(randInt(1, 72)),
  })),
];
